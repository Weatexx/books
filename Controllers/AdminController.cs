...
namespace books.Controllers.Admin
{
    ...
    public class AdminController : Controller
    {
        ...
        [HttpPost]
        public async Task<IActionResult> KitapEkle(KitaplarVM gelenData)
        {
            if (!ModelState.IsValid)
            {
                return View(gelenData);
            ...
            {
                Adi = gelenData.adi,
                YazarId = gelenData.yazarID,
                DilId = gelenData.dilID,
                SayfaSayisi = gelenData.sayfaSayisi,
                YayineviId = gelenData.yayineviID,
                Ozet = gelenData.ozet,
                YayinTarihi = gelenData.yayinTarihi,
            };

            await db.AddAsync(yeniKitap);
            await db.SaveChangesAsync();
        ...
        {
            ViewBag.UserInfo = (from x in db.Kitaplars
                                 where x.Id == id
                                 select new KitaplarVM
                                 {
                                     id = x.Id,
        ...
        [HttpPost]
        public async Task<IActionResult> KitapDuzenle(KitaplarVM gelendata)
        {
            var kitap = await db.Kitaplars.FirstOrDefaultAsync(x => x.Id == gelendata.id);
            ...
            kitap.Id = gelendata.id;
            kitap.Adi = gelendata.adi;
            kitap.YazarId = gelendata.yazarID;
            kitap.DilId = gelendata.dilID;
            kitap.SayfaSayisi = gelendata.sayfaSayisi;
            kitap.YayineviId = gelendata.yayineviID;
            kitap.Ozet = gelendata.ozet;
            kitap.YayinTarihi = gelendata.yayinTarihi;
        ...
        {
            ViewBag.KitapInfo = (from x in db.Kitaplars
                                 where x.Id == id
                                 select new KitaplarVM
                                 {
                                     id = x.Id,
